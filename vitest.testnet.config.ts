 ...
 *
 * It spends real testnet funds, so it is never part of `npm run verify`, never
 * runs on a pull request, and skips itself when its credentials are absent.
 * Run it with `npm run test:testnet`, or from the manual
