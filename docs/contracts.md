...
- **Additive:** `PaymentMethodName` gains `'mpp'`, plus `PAYMENT_METHOD_NAMES`, the values as a runtime array. *Use case:* MPP (Machine Payments Protocol) as a second payment rail beside x402. *Scope:* the name only - no MPP wire type enters core and no frozen shape changed, because a rail's own vocabulary already travels through fields core carries without interpreting: `PaymentChallenge.accepts` and `envelope`, `PaymentSubmission.payload`, `PaymentResult.externalReference`, and `replayKey`, which the pipeline reserves as an opaque string. *Why a value export too:* config was carrying its own `new Set(['x402'])`. The type stays hand-written rather than derived from the array, for the reason the `PROTOCOL_NAMES` entry above records: a derived alias expands into a literal union at every use site of the printed contract surface. The annotation rejects an entry that is not a `PaymentMethodName`, but nothing makes the list exhaustive. *Resource level:* `payments: [x402, mpp]` is an ordered candidate list, not a choice offered to the buyer. `pickProvider` selects the first method with a configured provider, and that provider challenges, verifies and settles; there is no retry loop, so a rejected payment is never re-offered on the other rail. *Lifecycle:* config load refuses a paid resource unless at least one of its named rails is configured and enabled, which generalises the per-method x402 gate it replaces. The only input whose treatment changes is a paid resource naming `mpp` alone: refused, since no provider is built for it yet. The composition root's equivalent check therefore stays a drift detector, and the pipeline independently guards custom constructions with `PAYMENT_PROVIDER_UNAVAILABLE`. *Compatibility:* additive union member. Nothing in-tree switches on a method name, though an outside consumer switching exhaustively gains a case.
---
